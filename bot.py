...
from data_layer import (
    fetch_chain_snapshot as fetch_option_chain,
    fetch_expiry_list,
    check_margin,
    get_fund_limits,
    place_buy_order,
...
from order_manager import OptionLeg, monitor_positions
...
# Tick callback (called by WS parser)
...
# Capital sizing
async def compute_lots(ltp: float, lot_size: int) -> int:
    funds = await get_fund_limits()
...
async def strategy_loop() -> None:
    ...
    expiries = await fetch_expiry_list(NIFTY_UNDERLYING_ID)
    if not expiries:
    ...
    while is_market_open():
        ...
        if open_legs:
            await monitor_positions(open_legs, live_ticks)
        ...
        chain_data = await fetch_option_chain(NIFTY_UNDERLYING_ID, nearest_expiry)
        ...
        lots = await compute_lots(strike.ltp, NIFTY_LOT_SIZE)
        qty = lots * NIFTY_LOT_SIZE

        margin_data = await check_margin(strike.security_id, qty, strike.ask)
        ...
        order_id = await place_buy_order(strike.security_id, qty, strike.ask, order_type="LIMIT")
